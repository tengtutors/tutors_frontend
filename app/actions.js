...
/*
    Problem within Tiktok Unofficial API (Illegal as well)
*/
...
export async function generateArticle({ openaiAPI = "", tiktokURL = "" }) {
    ...
    try {
        ...
        const extractedText = await convertVideoToText(
            extractedVideoUrl,
            openai
        );
            ...
            messages: [
                {
                    role: "system",
                    content:
                        "You will generate an article based on the context provided",
                },
...
async function extractVideoFromTikTokVideo(videoUrl) {
    try {
        ...
        const extractedVideoUrl =
            resData.aweme_list[0].video.play_addr.url_list[0];
